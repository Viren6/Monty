...
// This list contains 1858 moves (plus one potentially if copied incorrectly, so we use a slice)
pub const LC0_MOVES: &[&str] = &[
    ...
    "h4h5",  "h4f6",  "h4g6",  "h4h6",  "h4e7",  "h4h7",  "h4d8",  "h4h8",  "a5a1",  "a5e1",  "a5a2",  "a5d2",  "a5a3",  "a5b3",  "a5c3",  "a5a4",
    ...
    "g5h7",  "g5d8",  "g5g8",  "h5d1",  "h5h1",  "h5e2",  "h5h2",  "h5f3",  "h5g3",  "h5h3",  "h5f4",  "h5g4",  "h5h4",  "h5a5",  "h5b5",  "h5c5",
...
fn idx_to_sq(i: u16) -> String {
    format!("{}{}", ((i & 7) as u8 + b'a') as char, (i / 8) + 1)
}

pub fn get_lc0_index(mov: &Move) -> Option<usize> {
    let indices = LC0_INDICES.get_or_init(|| {
        let mut map = HashMap::new();
        for (i, &m) in LC0_MOVES.iter().enumerate() {
            map.insert(m.to_string(), i);
        }
        map
    });
...
pub fn monty_move_to_lc0_index(mov: Move, pos: &ChessState) -> Option<usize> {
    ...
    let lookup_move = if stm == 1 {
        let src = mov.src() ^ 56;
        let dst = mov.to() ^ 56;
        Move::new(src, dst, mov.flag())
    } else {
        mov
    };

    // Map castling to king-takes-rook (FRC style) for LC0
    ...
    let lookup_move = if flag == 2 || flag == 3 {
        ...
        if is_rook {
            ...
            Move::new(u16::from(lookup_move.src()), rook_file, flag)
        }
    } else {
        lookup_move
    };

    get_lc0_index(&lookup_move)
}
