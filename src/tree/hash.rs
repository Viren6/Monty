...
impl HashTable {
    pub fn new(size: usize, topology: &ThreadTopology) -> Self {
        ...
        if size > 0 {
            unsafe {
                ...
                let chunk_size = (size * size_of::<HashEntryInternal>())
                    .div_ceil(init_threads)
                    .max(1);
    ...
    pub fn clear(&mut self, topology: &ThreadTopology) {
        if self.table.is_empty() {
            return;
        }

        let init_threads = topology.init_binding_count();
        let chunk_size = self.table.len().div_ceil(init_threads).max(1);

        std::thread::scope(|s| {
            for (chunk_idx, chunk) in self.table.chunks_mut(chunk_size).enumerate() {
                let binding = topology.init_binding(chunk_idx);
                s.spawn(move || {
                    if let Some(binding) = binding {
                        binding.apply();
