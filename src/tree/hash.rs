...
impl PolicyEntry {
    pub fn ptr(&self) -> super::node::NodePtr {
        super::node::NodePtr::from_raw(self.ptr)
    }
}

#[derive(Default)]
struct PolicyEntryInternal(std::sync::atomic::AtomicU64);
...
impl PolicyTable {
    ...
    fn fetch(&self, hash: u64) -> PolicyEntry {
        if self.table.is_empty() {
            return PolicyEntry::default();
        }
        let idx = hash % (self.table.len() as u64);
        PolicyEntry::from(&self.table[idx as usize])
    }

    pub fn get(&self, hash: u64) -> Option<super::node::NodePtr> {
        let entry = self.fetch(hash);
        if entry.hash == Self::key(hash) {
            Some(entry.ptr())
        } else {
    ...
    pub fn push(&self, hash: u64, ptr: super::node::NodePtr) {
        if self.table.is_empty() {
            return;
        }
        let idx = hash % (self.table.len() as u64);
